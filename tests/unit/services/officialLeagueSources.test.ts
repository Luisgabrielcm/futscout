...
test("only the five exact owner-approved official league tuples extend the 40 API leagues", async () => {
  assert.deepEqual(PREPARED_OFFICIAL_LEAGUE_ALLOWLIST.map(row => row.entityId), sources.OFFICIAL_LEAGUE_SOURCES.map(row => row.entityId))
  assert.equal(BRAND_ASSET_PILOT_ALLOWLIST.length, 622)
  assert.equal(BRAND_ASSET_PILOT_ALLOWLIST.filter(row => row.entityType === "LEAGUE").length, 45)
  const r = reference()
  ...
  assert.deepEqual(BRAND_ASSET_PILOT_ALLOWLIST.filter(row => row.entityType === "LEAGUE" && row.provider !== "api-football").sort((a, b) => a.entityId.localeCompare(b.entityId)), [...PREPARED_OFFICIAL_LEAGUE_ALLOWLIST].sort((a, b) => a.entityId.localeCompare(b.entityId)))
