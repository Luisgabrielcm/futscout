...
test("closed 40-club and 2-league dry-run records owner authorization but stays blocked by unverified delivery", () => {
  const result = prepareBrandAssetPilotDryRun(currentPilot(), now)
  assert.equal(result.length, 42)
  assert.equal(result.filter(row => row.identity.entityType === "CLUB").length, 40)
...
test("allow-list rejects a forty-third candidate, replacement, reordering and Club/League type mismatch", () => {
