...
test("closed 574-club and 2-league dry-run records owner authorization but stays blocked by unverified delivery", () => {
  const result = prepareBrandAssetPilotDryRun(currentPilot(), now)
  assert.equal(result.length, 576)
...
test("allow-list rejects a 577th candidate, replacement, reordering and Club/League type mismatch", () => {
