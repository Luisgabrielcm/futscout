...
test("closed 574-club and 7-league dry-run records owner authorization but stays blocked by unverified delivery", () => {
  const result = prepareBrandAssetPilotDryRun(currentPilot(), now)
  assert.equal(result.length, 581)
  assert.equal(result.filter(row => row.identity.entityType === "CLUB").length, 574)
  assert.equal(result.filter(row => row.identity.entityType === "LEAGUE").length, 7)
...
test("allow-list rejects a 582nd candidate, replacement, reordering and Club/League type mismatch", () => {
