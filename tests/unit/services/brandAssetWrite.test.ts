...
test("closed 200-club and 2-league dry-run records owner authorization but stays blocked by unverified delivery", () => {
  const result = prepareBrandAssetPilotDryRun(currentPilot(), now)
  assert.equal(result.length, 202)
  assert.equal(result.filter(row => row.identity.entityType === "CLUB").length, 200)
...
test("allow-list rejects a 203rd candidate, replacement, reordering and Club/League type mismatch", () => {
