...
test("closed 100-club and 2-league dry-run records owner authorization but stays blocked by unverified delivery", () => {
  const result = prepareBrandAssetPilotDryRun(currentPilot(), now)
  assert.equal(result.length, 102)
  assert.equal(result.filter(row => row.identity.entityType === "CLUB").length, 100)
...
test("allow-list rejects a 103rd candidate, replacement, reordering and Club/League type mismatch", () => {
