...
### Controle independente de liberação

A ausência de Brack na allowlist impede o **writer**, não a entrega de um asset já autorizado no Registry. No commit original `55382e9`, o 404 antes do cadastro era uma condição do Registry. Para impedir entrega antecipada mesmo que um asset autorizado apareça, a rota agora exige a variável exclusivamente de servidor `BRACK_BRAND_ASSET_DELIVERY_ENABLED` com valor exato `true`. Ausência, `false`, valores vazios ou diferentes mantêm o controle desligado. Nenhuma variável local ou da Vercel foi alterada.

Desligado: 404 vazio, `no-store`, antes do limitador, banco e rede. Ligado: continuam obrigatórios Registry elegível, autorização operacional, política global e ausência de bloqueio/revogação; a flag não autoriza o cadastro nem contorna esses controles. Testes exercitam os dois estados com asset sintético autorizado, além do GET real com dependências mockadas e Registry vazio. O fallback existente trata a resposta sem imagem. A flag não afeta API-Football.

No primeiro release, manter esta variável ausente/desligada em Preview e Production. A futura habilitação exige autorização separada e um novo deployment com a variável de servidor correta; não presumir mudança instantânea do ambiente de um deployment existente. Somente depois da confirmação independente do futuro cadastro e da habilitação deve-se esperar PNG público e cobertura 41/45. Para suspensão, retirar a habilitação em um deployment posterior; para revogação individual imediata, o Registry continua sendo o controle por asset. O modelo de volume abaixo só se aplica com a flag ligada; desligada, a rota faz zero consultas e zero downloads.
...
2. Publicar este código **com a allowlist atual**, sem Brack, e com o controle de entrega desligado. Confirmar a revisão implantada, GET `/api/brand-assets/brack` retornando 404 e as 40 logos atuais no catálogo. Com a entrega habilitada futuramente, ausência de asset elegível produz 404; falha de leitura produz 502 e deve ser investigada. O 404 com a flag desligada não comprova ausência de asset: consultar o Registry separadamente.
...
8. Após commit, leitura independente deve confirmar identidade VERIFIED/v1 e LOGO ACTIVE/v1, URL/hash/decisão corretos, nenhuma alteração nos registros anteriores e hashes protegidos iguais. Em liberação separadamente aprovada, habilitar a entrega no deployment e confirmar resposta PNG/hash da rota pública e imagem/fallback na UI a 24/48/72 px. Só então registrar cobertura **41/45**; as outras quatro continuam pendentes. Se a imagem falhar, registrar falha e não substituir por outra fonte.
