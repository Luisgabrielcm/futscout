...
### Conclusão ISL posterior ao checkpoint

Correção publicada no SHA `aeb7fc6f1ddbe3f4998aaeeede327b7ca5ca0a69`: Preview `dpl_BaDTXJvbMMZVtLZ9EZciD3FrrZ33`, Production inicial `dpl_ByE9Qbps6HwHrdEmSmpEmcCrCj2u`. Preflight novo `isl-approved-preflight-v2-20260924.json` comparado integralmente ao estado após Brack, sem divergências. Nova operação controlada após diagnóstico/reconciliação: `audit/output/isl-registration-approved-v2-20260924.json`, **COMMITTED_INDEPENDENT_READ_CONFIRMED**. Identidade `cmufqai9d0000oouc4ikhsj0r` VERIFIED v1, asset `cmufqaidw0001ooucykj66tsu` ACTIVE v1. Hash SVG exato confirmado; rightsStatus REVIEW_REQUIRED, storageUrl=null, revocable=true.

Flag ISL habilitada individualmente apenas em Production; deployment do mesmo SHA `dpl_3JYBui1iTdzebXCHjeKvU8rtV2Fr`, URL imutável `https://futscout-e1zkikkia-fut-scout.vercel.app`. Em 24/09, a página pública `/pt/ligas/isl` exibiu as 12 instâncias da logo via `/api/brand-assets/isl`, todas carregadas em 74×74 naturais, com inspeção visual do cabeçalho/cards. **42/45 confirmadas publicamente**. Rollback compatível anterior: `dpl_ByE9Qbps6HwHrdEmSmpEmcCrCj2u` (ISL desligada, Brack ligada), sem apagar o Registry. Nenhuma configuração de conexão da Vercel foi alterada.

Gates do candidato aeb7fc6: 1575 testes aprovados, oito HTTP estritos executados separadamente no fluxo anterior; TypeScript, lint, build do SHA exato e diff check aprovados. As 40 logos API-Football foram conferidas carregadas no Preview (21 + 19) antes da promoção. Preview usa o banco compartilhado e não foi tratado como homologação isolada.
